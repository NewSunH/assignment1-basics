...
implemented in \texttt{/cs336\_basics/experiment\_logging.py}. The logger writes per-step metrics with both \texttt{step} and \texttt{wallclock\_s} to \texttt{metrics.jsonl}, and also stores run metadata (\texttt{run.json}) and free-form notes (\texttt{notes.log}). My running experiment log is maintained in \texttt{/experiments/experiment\_log.md}.
\end{problem}
