      ...
      \item Perform a hyperparameter sweep over the learning rates and report the final losses (or note divergence if the optimizer diverges).\\
            ...
            \begin{center}
                  \begin{tabular}{lcc}
                        ...
                        \midrule
                        \texttt{1e-4}    & \texttt{2.7623}       & converged                     \\
                        \texttt{2e-4}    & \texttt{2.0991}       & converged                     \\
                        \texttt{3e-4}    & \texttt{1.8096}       & converged                     \\
                        \texttt{5e-4}    & \texttt{1.5811}       & converged                     \\
                        \texttt{8e-4}    & \texttt{1.4690}       & converged                     \\
                        \texttt{1e-3}    & \texttt{1.4368}       & converged (meets $\le 1.45$)  \\
                        \texttt{3e-3}    & \texttt{1.3713}       & best run                      \\
                        \texttt{3e-2}    & \texttt{4.3973}       & does not learn well           \\
                        \texttt{3e1}     & ---                   & diverged at step \texttt{188} \\
                        \bottomrule
                  \end{tabular}
            \end{center}
